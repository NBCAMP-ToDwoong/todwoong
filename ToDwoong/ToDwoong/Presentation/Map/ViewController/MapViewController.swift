...
extension MapViewController {
    private func addPinsToMap(todos: [TodoType]) {
        ...
        if let selectedGroupIndex = self.selectedGroup {
            let selectedGroupTitle = self.groupList[selectedGroupIndex].title
            filteredTodos = self.allTodoList.filter { $0.group?.title == selectedGroupTitle }
        } else {
            filteredTodos = self.allTodoList
        }
        
        for todo in filteredTodos {
            if let latitude = todo.placeAlarm?.latitude, let longitude = todo.placeAlarm?.longitude {
                let pin = self.createAnnotation(title: todo.title,
                                                latitude: latitude,
                                                longitude: longitude,
                                                pinColor: UIColor(hex: todo.group?.color ?? "#000000"))
                self.mapView.mapView.addAnnotation(pin)
                self.pins.append(pin)
            }
        }
        
        self.mapView.mapView.showAnnotations(self.pins, animated: true)
    }
